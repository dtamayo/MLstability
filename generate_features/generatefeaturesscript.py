...
lyapwindow = int(sys.argv[4])         # number of points to take for the averaging in the Lyapunov time calculation
...
def generate_features(sim):
    ...
    Nfit = N//lyapwindow # N of binned points
    Ntrim = Nfit*lyapwindow # trim array to number that can fit in bins of size lyapwindow
    e2avg = np.average(e[2][:Ntrim].reshape(Nfit, lyapwindow), axis=1) # reshape into Nfit lists of lyapwindow consecutive values and average
    e2shadowavg = np.average(e2shadow[:Ntrim].reshape(Nfit, lyapwindow), axis=1)
    timesavg = np.average(times[:Ntrim].reshape(Nfit, lyapwindow), axis=1)
...
def dorows(params):
    ...
    for i in range(start+1,end):
        ...
        df_full.to_csv('../csvs/tmp/sifOrbs={0}Nout={1}window={2}lyapwindow={3}'.format(maxorbs, Nout, window, lyapwindow)+str(start)+'.csv', encoding='ascii')
...
dfs = []
for start in starts:
    dfs.append(pd.read_csv('../csvs/tmp/sifOrbs={0}Nout={1}window={2}lyapwindow={3}'.format(maxorbs, Nout, window, lyapwindow)+str(start)+'.csv', index_col=0))
df = pd.concat(dfs)
df.to_csv('../csvs/sifOrbs={0}Nout={1}window={2}lyapwindow={3}.csv'.format(maxorbs, Nout, window, lyapwindow), encoding='ascii')
